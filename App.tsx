      ...
      const unsubClasses = subscribeToClasses((firebaseClasses) => {
        ...
        if (Object.keys(firebaseClasses).length > 0) {
          ...
          Object.keys(migratedClasses).forEach(id => {
            ...
            // Clean up Turma 802 to contain the correct 30 students with their presence/absence records (including 03/08)
            if (migratedClasses["802"]) {
              const correctStudents802: any[] = initialClassData["802"].students;

              const studs802 = migratedClasses["802"].students || [];
              const hasAttendanceMismatch802 = studs802.length !== 30 || studs802.some((s: any) => {
                const corr = correctStudents802.find(c => c.id === s.id);
                if (!corr) return true;
                return !s.attendance || !s.attendance["03/08"];
              });

              if (hasAttendanceMismatch802) {
                migratedClasses["802"].students = correctStudents802;
                if (initialClassData["802"].dailyActivities) {
                  migratedClasses["802"].dailyActivities = initialClassData["802"].dailyActivities;
                }
                needsUpdateRemote = true;
              }
            }
